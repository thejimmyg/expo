...
import static host.exp.exponent.ExponentManifest.MANIFEST_SPLASH_IMAGE_URL;
...
public class ExperienceSplashScreenConfig {
  ...
  @Nullable private static String parseImageUrl(@Nullable JSONObject manifest) {
    return getStringFromManifest(
      manifest,
      new String[] { MANIFEST_ANDROID_INFO_KEY, MANIFEST_SPLASH_INFO_KEY, MANIFEST_SPLASH_IMAGE_URL },
      new String[] { MANIFEST_SPLASH_INFO_KEY, MANIFEST_SPLASH_IMAGE_URL }
    );
  }

  @Nullable private static String getStringFromManifest(@Nullable JSONObject manifest, String[] path, String[] fallbackPath) {
    String pathResult = getStringFromManifest(manifest, path);
    if (pathResult != null) {
      return pathResult;
    }
    return getStringFromManifest(manifest, fallbackPath);
  }
